...
**/
tBTNodePtr selectNode = NULL;
tBTNodePtr node = NULL;
selectNode = (*RootPtr);
tBTNodePtr mynode=NULL;

while(selectNode != NULL)
{
	mynode = selectNode;
	if(Content < selectNode->Cont)
	{
		selectNode = selectNode->LPtr;
	}
	else if (Content > selectNode->Cont)
	{
		selectNode = selectNode->RPtr;
	}
	else
	{
		return;
...
if(Content < mynode->Cont)
{
	mynode->LPtr = node;
}
else if (Content > mynode->Cont)
{
	mynode->RPtr = node;
}
else
...
while (SEmptyP(&myStack) != 1)
{
	RootPtr = STopPopP(&myStack);
	Leftmost_Preorder(RootPtr->RPtr,&myStack);
}
...
while (SEmptyP(&myStack) != 1)
{
	...
	Leftmost_Inorder(RootPtr->RPtr,&myStack);
}
}
...
///SPushB the "1" means TRUE = It was once visited
while (ptr != NULL)
{
	SPushP(StackP,ptr);
	SPushB(StackB,1);
...
tStackP myStackP; tStackB myStackB;
SInitB(&myStackB);SInitP(&myStackP);

Leftmost_Postorder(RootPtr,&myStackP,&myStackB);

while(SEmptyP(&myStackP) != 1)
{

	RootPtr = STopPopP(&myStackP);

	if(SEmptyB(&myStackB) != 1)
	{
		if(STopPopB(&myStackB) == 1)
		{
			SPushP(&myStackP,RootPtr);
			SPushB(&myStackB,0);
			Leftmost_Postorder(RootPtr->RPtr,&myStackP,&myStackB);
			continue;
		}
	}
...
while(((*RootPtr) != NULL) && (SEmptyP(&myStack) != 1))
{
	node = (*RootPtr);
	(*RootPtr) = STopPopP(&myStack);
	if((*RootPtr)->LPtr)
	{
		SPushP(&myStack,(*RootPtr)->LPtr);
	}

	if((*RootPtr)->RPtr)
	{
		SPushP(&myStack,(*RootPtr)->RPtr);
	}
